...
function SectionCard({ children }: { children: React.ReactNode }) {
    return (
        <View className="rounded-xl border border-border bg-surface-secondary p-4">{children}</View>
    )
}

function TakeoutIdleState({
    isVisible,
    onSelectFiles,
}: {
    isVisible: boolean
...
function MailboxWarning({ isVisible }: { isVisible: boolean }) {
    ...
    return (
        <Text className="text-danger" style={{ fontSize: 13 }}>
            Mail was found but you don't have a mailbox set up. Configure a mailbox in Mail settings
            first to import mail.
        </Text>
    )
}
...
function TakeoutCompleteState({
    ...
    return (
        <View className="gap-3 items-center py-4">
            ...
            <Pressable
                onPress={onReset}
                className="px-4 py-2.5 rounded-lg border border-muted-foreground mt-2"
            >
                <Text className="text-foreground" style={{ fontSize: 14 }}>
                    Import More
                </Text>
